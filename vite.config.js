...
function mockRelay() {
  ...
  // 2026-07-27 -- not synthesized. Returned for any requested date (a mock
  // simplification -- the real endpoint varies by date, this doesn't), same
  // convention as the other mocks here trading per-input variation for a
  // real, unaltered captured shape.
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        if (req.url?.startsWith('/archive/query')) {
          res.setHeader('Content-Type', 'application/json')
          return res.end(JSON.stringify(archiveQueryMock))
        }
        if (req.url === '/health') {
