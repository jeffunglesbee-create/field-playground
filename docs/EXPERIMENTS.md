...
| [`production-gaps-2026-07-25`](EXPERIMENT-production-gaps-2026-07-25.md) | **Done (4/5 live-confirmed)** | Watchlist and game-transition toast (`<Portal>`, first use in this repo) both confirmed via real browser checks. Stale indicator and tonight's-card aggregate build-verified. Date browser build-verified only, lowest-risk/most mechanical of the five. Real finding: a plain `createSignal(new Set())` would silently fail to trigger updates on `.add()`/`.delete()` — `createStore` doesn't have that trap, documented directly in the watchlist code. |
...
back to the repo. Now covers nine checks across five experiments in one
run. Extend this harness before reaching for a new one.
